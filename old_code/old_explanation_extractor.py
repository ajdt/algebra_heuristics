...
class ExplanationTemplate(object):
    ...
    def translateVarsToTemplateNames(template, var_assignment, old_variables):
        ...
        substituted             = substituteVariableValues(old_variables, var_assignment)
        new_vars_with_values    = zip(substituted, ExplanationTemplate.getPredicateVariables(template.rule.head))
        
        # remove cases where no assignment was found, and create new dictionary
        return dict(filter(lambda tup: tup[0] != None, new_assign_tuples))
    ...
    def unify(var_assignment, conditions, model_mgr):
        ...
        print 'condition name, ', first_cond.name
        if first_cond.name == 'isTheDegreeOf':
            print cond_key
            print model_mgr.model_predicates.has_key(cond_key)
        ...
        if depth <= 1:
            ...
            for pred in filterUnusedConditions(self.rule.body):
                if self.manager.hasExplanationForPredicate(pred):
                    ...
                    translated_assign   = ExplanationTemplate.translateVarsToTemplateNames(pred_definition, var_assignment, old_variables)
